angular.module('controller',[])
    ...
    .controller('MController', ['$scope', '$http', '$log', function($scope, $http, $log) {
        ...
        $scope.MainMission = function($mission)
        {
            succes = $scope.checkMissionSucces($mission);
            if (succes == true || succes == false)
            {
                stmt = $scope.checkLVLUp(succes);
                $scope.updateCharacter(stmt);
            }
        }

        $scope.checkMissionSucces = function($mission)
        {
            user = $scope.user[0];
        ...
        $scope.checkLVLUp = function (succes)
        {
            ...
            if (succes == true) {
                ...
                if ((parseInt(user.EXP) + parseInt($mission.EXP_Reward)) >= 100) {
                    ...
                    att = parseInt(parseInt(user.Att) + 10);
                    def = parseInt(parseInt(user.Def) + 5);
